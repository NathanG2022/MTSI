...
#NAME OF USER & SETUP THE TEXT THAT WILL BE READ OUT
name = ""
if(len(name) == 0 ):
    audio = "Wake Up!" 
else:
...
dlib_facelandmark = dlib.shape_predictor("C:/HomeRepos/M&TSI/Drowsy/shape_predictor_68_face_landmarks.dat")
...
EYE_CLOSED_THRESHOLD = 5 # Number of frames eyes need to be closed to trigger alert
SLEEPY_THRESHOLD = 0.2  # Eye aspect ratio threshold to consider eyes as closed
...
while True:
    ...
    for face in faces:
        ...
        # DEPENDING ON THE SIZE OF THE EYES IT WILL DECIDE WHETHER THE PERSON'S EYES ARE CLOSE OR NOT
        if Eye_Rat < SLEEPY_THRESHOLD:
            eye_closed_counter += 1
        else:
            eye_closed_counter = 0

        if eye_closed_counter >= EYE_CLOSED_THRESHOLD:
            cv2.putText(frame, "Sleepy", (50, 100),
